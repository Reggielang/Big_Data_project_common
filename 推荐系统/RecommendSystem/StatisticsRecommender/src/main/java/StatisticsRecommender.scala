...
object StatisticsRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //todo: 构建一张临时表之后就可以用sparkSQL做不同的统计推荐
    ...
    //注册UDF，将timestamp转换为yyyyMM格式
    spark.udf.register("changeDate",(x:Int)=>simpleDateFormat.format(new Date(x*1000L)).toInt)

    //把ratings数据转换成想要的结构productId,score,yearmonth
    val ratingOfYearMonthDF = spark.sql("select productId,score,changeDate(timestamp) as yearmonth from ratings")
    ratingOfYearMonthDF.createOrReplaceTempView("ratingOfMonth")

    val rateMoreRecentlyProductsDF = spark.sql("select productId, count(productId) as count,yearmonth from ratingOfMonth " +
