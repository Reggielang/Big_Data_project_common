...
object OnlineRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //对Kafkastream进行处理产生评分流，userId|PrductId|score|timestamp
    ...
    ratingStream.foreachRDD{
      rdds=>rdds.foreach{
        case(userId,productId,score,timestamp)=>
          ...
          //todo: 核心算法部分
          // 1. 从redis里取出，当前用户的最近评分，保存为一个数组Array[(productId,score)]
          val userRecentlyRatings = getUserRecentlyRatings(MAX_USER_RATING_NUM,userId,ConnHelper.jedis)

          // 2. 从相似度矩阵中获取，当前商品最相似的备选商品列表Array[productId]
          val candidatesProducts = getTopSimProducts(MAX_SIN_PRODUCTS_NUM,productId,userId,simProductMatrixBC.value)

          // 3. 计算每个备选商品的推荐优先级，得到当前用户的实时推荐列表Array[(productId,score)]
          val streamRecs = computeProductScore(candidatesProducts,userRecentlyRatings,simProductMatrixBC.value)
  ...
  /*
  获取当前商品的相似度列表，并过滤掉，用户已经评分过的，作为备选列表
   */
  ...
  //计算每个备选商品的推荐得分
  def computeProductScore(candidatesProducts: Array[Int],
                          userRecentlyRatings: Array[(Int, Double)],
                          simProducts:scala.collection.Map[Int, scala.collection.immutable.Map[Int, Double]])
  :Array[(Int,Double)]={
    //定义一个长度可变数组ArrayBuffer,用于保存每一个备选商品的基础得分
    val scores = scala.collection.mutable.ArrayBuffer[(Int,Double)]()
    //定义两个map，用于保存每个商品的高分和低分的计数器,productId->count
    val increMap = scala.collection.mutable.HashMap[Int,Int]()
    val decreMap = scala.collection.mutable.HashMap[Int,Int]()
    //遍历每个备选商品，计算和已评分商品的相似度
    for(candidatesProduct<- candidatesProducts; userRecentlyRating <- userRecentlyRatings){
      //从相似度矩阵中获取当前备选商品和当前已评分间的相似度
      val simScore = getProductsSimScore(candidatesProduct,userRecentlyRating._1,simProducts)
      if(simScore>0.4){
        //按照公式进行加权计算,得到基础评分
        scores+=((candidatesProduct,simScore*userRecentlyRating._2))
        if (userRecentlyRating._2>3){
          increMap(candidatesProduct)=increMap.getOrDefault(candidatesProduct,0)+1
        }else{
          decreMap(candidatesProduct)=increMap.getOrDefault(candidatesProduct,0)+1
        }
      }
    }
